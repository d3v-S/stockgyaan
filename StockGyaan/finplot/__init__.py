...
class FinViewBox(pg.ViewBox):
    def __init__(self, win, init_steps=300, yscale=YScale('linear', 1), v_zoom_scale=1, restore = False, onClickHandler=None, ctrlOnClickHandler=None,  *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.win = win
        ...
        self.on_click_handler = onClickHandler
        self.ctrl_click_handler = ctrlOnClickHandler
        self.reset()
    ...
    def mouseLeftDrag(self, ev, axis):
        ...
        if self.ctrl_click_handler is not None:
            self.ctrl_click_handler(ev, ev.pos())
        ev.accept()
    ...
    def mouseClickEvent(self, ev):
        ...
        if ev.button() != QtCore.Qt.LeftButton or ev.modifiers() != QtCore.Qt.ControlModifier or not self.draw_line:
            if self.on_click_handler is None:
                ...
                self.on_click_handler(ev, self.mapToView(ev.pos()))
                ev.accept()
                return
...
def create_plot_widget(master, rows=1, init_zoom_periods=1e10, yscale='linear',
                       restore=False, onClickHandler=None, ctrlOnClickHandler=None):
    pg.setConfigOptions(foreground=foreground, background=background)
    global last_ax
    ...
    for n in range(rows):
        ...
        ysc = YScale(ysc, 1)
        viewbox = FinViewBox(master, init_steps=init_zoom_periods,
                             yscale=ysc, v_zoom_scale=1-y_pad,
                             enableMenu=False,
                             restore=restore,
